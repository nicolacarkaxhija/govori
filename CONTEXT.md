...
- **Instance** — one deployable product as configuration in
  `instances/*`: branding, UI catalog set, learner-language roster, and
  the directions it hosts. Selected explicitly at build/boot; there is
  no default (ADR 0029/0041).
- **Direction** — one learning offering an instance hosts: a language
  pack plus per-direction tuning (own-language label, translation
  fallback, community publish threshold). Content rows are scoped by
  direction; an omitted direction id resolves only when the instance
  declares exactly one — a total function of config, never a default
  (ADR 0046).
