...
| 2026-07-19 | feat: scope stored content by direction | 4–5 | items/units/review_queue gained a direction column (migration 0012, generated clean, deliberately nullable — static SQL cannot know a deployment's directions, so both composition roots run backfillDirections stamping NULL rows with the first direction right after migrating and adapters treat NULL as absent); every content/course/stats/review/export port and drizzle adapter now scopes by direction, approvals and vote publications land in the draft's own queued direction, importArtifact and the import CLI take --direction (omittable only where a single-direction instance makes it total); integration suite covers backfill idempotence and cross-pool isolation |

| 2026-07-19 | feat: resolve the direction per request in the api | 3–4 | buildApp dropped its boot-time sole direction for a per-request directionFor helper over resolveDirection (total when the instance declares one direction, 400 naming the known ids otherwise); /items, /stats, /course, /lessons/:id(+/sentences) and all three /export routes take an optional direction query param that becomes required with a second direction; /contribute takes it in the body and validates against that direction's own pack schema and orthography name; the vote publish threshold now comes from the pending draft's direction (stranded drafts never auto-publish); response schemas validate over the union of hosted packs; new two-direction fixture suite covers 400s, scoping, per-direction contribution wording, and thresholds |

| 2026-07-19 | feat: thread the working direction through the web app | 4–5 | instance.ts now exposes the direction roster with a persisted active direction (localStorage per instance, first declared direction as the config-owned primary) behind activeDirection/activePack/fallbackLang/setActiveDirection; new useDirection hook and a topbar switcher rendered only when two directions exist (shell keyed by direction so views refetch); useScript/useLearnLanguage re-read against the active pack and fallback on a switch; every card and view moved from the static pack export to render-time activePack(); api client and all fetching views pass the direction (items, course, lessons, sentences, stats, contribute incl. free production); switchDirection key across all seven catalogs; new useDirection and two-direction shell suites, web 92.4/87.7/90.5/93.0 |
...
| 2026-07-19 | feat: dress govori in Frost and fol in Seed | 3–4 | The two chosen identities from the design explorer, ported faithfully: Govori = Glassmorphism·Frost (icy blue canvas gradient, blurred translucent surfaces, luminous hairlines, a true-red primary in both modes, the stitch sealed in a frosted pill) and Fol = Material You·Seed (red-seeded tonal neutrals, M3 pill actions and rounded chips, no borders/shadows, the stitch as a primary-container chip); full light+dark token sets with color-mix hover/soft derivations; the engine gained family-agnostic structural tokens (--card-bg/-border/-blur/-shadow, --control-bg/-border, --app-bg-image, --font-display) so base/learn stay token-only while each family's inherently specific CSS lives in its theme; both VITE_INSTANCE builds bundle their own theme |
| 2026-07-19 | fix: speak each catalog in its own language | 1–1.5 | Catalog purity sweep of both instances: the five feedback toasts (allDone/allReviewed/correct/incorrect/welcome) had been seeded with the target-language originals across catalogs that do not speak them. Fixed fol en (Albanian→English: Correct!/Not quite./Welcome./All done./All reviewed.), fol de (→German: Richtig!/Nicht ganz./Willkommen./Alles fertig./Alles wiederholt.), fol it (→Italian: Esatto!/Non proprio./Benvenuti./Tutto fatto./Tutto ripassato.), fol tr (→Turkish: Doğru!/Tam değil./Hoş geldiniz./Hepsi tamam./Hepsi tekrar edildi.) and govori en (Interslavic→English); sq and isv keep them as their own tongue. New per-instance purity tests assert the non-native catalogs no longer echo the flavour; twenty-one web exercise assertions moved to the English feedback |
| 2026-07-19 | feat: gather preferences into a settings screen | 3–4 | New SettingsView reached from a home-footer gear, holding colour theme (light/dark/system), interface language, translation language, and — only when the active pack writes more than one script — the display script; the always-visible top-bar language/script/theme toggles and the footer translation-language select are gone (the direction switcher stays, ADR 0046). useTheme became tri-state (system clears the [data-theme] override), useLanguage/useScript gained direct setters and rosters; onboarding keeps its translation-language step; seven sorted i18n keys added across all seven catalogs; useTheme, App, single-script and direction-switch suites reworked to drive Settings, plus a focused SettingsView suite; web 92.2/87.5/90.3/92.8 |
| 2026-07-19 | test: assert the theme identities apply per instance | 0.5–1 | Screenshot-free acceptance: a per-instance suite reads each theme and asserts it defines the whole custom-property contract, answers both prefers-color-scheme and the forced [data-theme] override, and carries its family signature — Govori's icy #e9f1f8 canvas, 14px card blur, 18px radius and true-red #ff5062 dark primary; Fol's #fef7f6 seed, 24px radius, borderless tonal card and 999px M3 pills; both VITE_INSTANCE builds ship green |
| 2026-07-19 | feat: build the audio-program data layer and routes | 4–5 | ADR 0048 storage made real: recordings gained a pseudonymous speaker id (derived, never the user id), accent tag, device_meta, versioned three-grant consent, vote status, direction, and a deletion tombstone; new recording_votes, audio_credits (the casual-tier premium-time ledger), and dataset_manifests tables (migration 0013, generated clean); store extended with dataset-grade add, verified-only listing, vote/verify-once/credit, and a mine view; pure credit-policy and speaker-pseudonym modules unit-tested; upload route now requires the consent triple + device meta (400/401 paths), POST /audio/:id/vote verifies at the direction net threshold and credits the contributor, GET /audio/mine returns own clips + consents + ledger — every route flag-dark; real-Postgres roundtrip incl. accrual and no-double-credit |
...
| 2026-07-20 | feat: name the attestation tier as a reusable type | 0.3 | Extracted AttestationSchema/Attestation to module scope, matching the existing part-of-speech export pattern, so the api layer can type against it |
| 2026-07-20 | feat: persist the forge's quality signals | 1–1.5 | Attestation, difficulty, and per-sense translations now survive the database round trip instead of being silently dropped at the repository layer — closes the gap the forge's schema proposal opened |
...
| 2026-07-20 | feat(i18n): add the community-audio program strings | 1–1.5 | Twenty-one sorted keys added to all seven catalogs (govori en+isv, fol en+sq+de+it+tr) for the audio recording UI: the three-grant consent sheet (title, intro, the app/dataset/training labels with a one-line plain-English note each, the required badge, the "what happens to my voice" expandable, the agree action), the My-recordings view (title, empty state, the pending/verified/rejected status words, the credit-ledger heading/line/none), and the community voice-review nav label; ADR-0048-honest copy that never over-promises; both instances rebuilt, parity/sort/non-empty gates green |

| 2026-07-20 | feat: capture three-grant consent and device metadata on upload | 3.5–4.5 | AudioTools now gates a learner's first clip behind a one-time consent sheet (ADR 0048): three toggles — app use required and locked on, dataset and training opt-in and off by default — each with a one-line plain-English note, plus an expandable "what happens to my voice" disclosure (pseudonymous storage, separable grants, withdrawal drops future dataset versions but shipped ones are non-recallable, never single-speaker cloning). The choice persists locally keyed to charter version '1' and is not re-asked per clip, but a version bump re-shows it (new audioConsent module, zod-guarded, its own unit suite). uploadRecording rebuilt to the live contract — top-level mime + base64 data, a device object (client-measured durationMs, sample rate read from the track settings when reported), and the consent triple; new fetchMyRecordings/castAudioVote/fetchPendingAudio clients with unauthenticated-vs-unreachable handling; token-only consent-sheet styling keeps the theme-seam guard green; web 92.5/87.9/90.3/93.1 |

| 2026-07-20 | feat: show a learner their recordings and premium-time ledger | 2.5–3 | New MyRecordingsView (GET /audio/mine) reached from Settings, gated on the audio flag being live and a session existing: it lists a contributor's own clips with their validation status (awaiting votes / verified / not accepted) and the three consent grants each carries — shown, never editable — as ✓/— chips, and states the casual-tier premium-time ledger in plain ADR-0048-honest terms ("{seconds} seconds validated → {days} premium days earned", or an unembroidered none line), distinguishing unauthenticated (offer sign-in) from unreachable. Settings gained the gated entry; App resolves the audio flag once and threads audioOn/signedIn through; token-only styling holds the theme seam; web 92.4/87.9/89.9/93.0 |

| 2026-07-20 | feat: add a community voice-review queue | 2–2.5 | New AudioReviewView reuses the text VoteView pattern for audio (ADR 0048/0040): a signed-in learner plays each pending clip and votes it up or down through POST /audio/:id/vote, tallies and status taken from the server response, a clip dropping out of the queue the moment a vote verifies or rejects it; per-clip vote scoping, existing-vote marking, and the unauthenticated/unreachable/empty phases mirror the draft queue. Reached from a home-footer entry shown only when the audio flag is live and a session exists; App gained the view and gate. The queue reads from GET /audio/pending — the one listing endpoint not yet in the live API contract — and degrades to "unavailable" until it ships; web 92.2/87.9/89.4/92.7 |

| 2026-07-20 | feat: parse the attestation tier on learn items | 0.3–0.5 | learnItemSchema now keeps the forge's optional gold/silver/bronze attestation (ADR 0051) instead of stripping it; a bad grade fails the payload closed, an absent grade still parses — most of the live ISV pool has no tier yet |

| 2026-07-20 | feat: keep bronze items out of exercise distractors | 1–1.5 | excludeBronzeDistractors(pool, target) folded into buildChoices, buildReverseChoices, and buildMatching (ADR 0051): a single-corpus bronze gloss is never confusing filler for another item's question, though a bronze item stays valid as the studied target; undefined tiers are neutral and never gated, so the untiered live pool is unaffected — a round falls back to fewer choices rather than surface bronze |

| 2026-07-20 | feat: meet high-confidence vocabulary first in a lesson | 0.8–1.2 | prioritizeAttested stably sinks bronze-tier words to the back of a lesson pool (ADR 0051), wired at LessonView's pool assembly; a preference not an exclusion (a bronze-only lesson still runs), and it only orders the unseen tail — SRS due-ness is untouched, untiered items keep their place |

| 2026-07-20 | docs: charter app-side data-quality gating (adr 0051) | 0.5–1 | ADR 0051: the exact locked design — excludeBronzeDistractors folded into the three choice-based builders (bronze never filler, target exempt, matching board bronze-free), prioritizeAttested sinking bronze to a lesson's tail as a preference not an exclusion, and undefined-as-neutral as the load-bearing rule that keeps the untiered live pool working; registry row |

| 2026-07-20 | feat(api): report quality problems and escalate flagged items | 3.5–4.5 | New item_reports table (migration 0016) with a nullable reporter_id — anonymous reporting is a quality signal, not an identity action — and a flagged_at stamped once when the third open report on an item lands (transactional, idempotent under further reports); POST /items/:id/report takes {reason (4-value enum), comment?}, 404s an unknown item, else 202s and tags the report with the session when there is one. GET /admin/quality-flags (reviewer+) is a pure server-side aggregation over the already-synced review-event log and the reports table — no new per-learner tracking: it escalates items lapse-heavy past the charter bars (>50% 'again' with ≥10 graded events) or hand-reported past three open reports, each with its counts and reason breakdown, most severe first. Threshold constants named in one module; Testcontainers proof of the flag-on-third and the two-signal query; api 96.7/90.9/96.1/96.9 |
| 2026-07-20 | feat(i18n): add the quality-feedback loop strings | 1–1.5 | Fifteen sorted keys added to all seven catalogs (govori en+isv, fol en+sq+de+it+tr) for the learner report affordance (the "Report this" opener, the "what looks wrong" prompt, the four reason labels, the optional-note label, cancel/send actions, and the non-blocking "a reviewer will take a look" thanks) and the reviewer flags view (title, empty and unavailable states, the lapse-rate line with its {rate}/{again}/{total} params, and the {count} open-reports line); natural per-language copy, both instances rebuilt, parity/sort/non-empty gates green |
| 2026-07-20 | feat(web): report items and review the quality flags | 3–4 | A reusable ReportControl seams into the item-based cards' feedback areas (Exercise, Listening, Morphology): a compact "Report this" opener expands to the four reasons plus an optional note, sends through reportItem — anonymously or signed-in — and swaps to a brief thanks, never blocking the learner even when the request fails. A new reviewer-reachable QualityFlagsView (from the account screen, mirroring ReviewView) lists GET /admin/quality-flags — each escalated item's text/translation in the chosen script with its lapse rate or open-report count and reason breakdown, most severe first — displaying rather than linking, since no per-item content-edit flow exists yet (ADR 0051 charter); reportItem/fetchQualityFlags clients drop a blank comment and distinguish forbidden/unreachable; token-only styling holds the theme seam; web 92.3/87.9/89.5/92.8 |
| 2026-07-20 | feat(api): add golden-set tables and migration | 0.5–1 | Two tables for the quality benchmark (ADR 0051): golden_sample (direction, item_id, added_at) — the fixed, append-only per-direction sample — and golden_audits (item_id, direction, reviewer_id, accuracy/naturalness/fit, comment, audited_at) with a one-row-per-(item,reviewer) primary key; both cascade off items. Drizzle migration 0016 and its snapshot generated from the schema |
| 2026-07-20 | feat(api): add the stratified golden-set sampler | 1–1.5 | Pure selectGoldenSample(pool, target) (ADR 0051): strata by kind × attestation-tier (untiered items in their own stratum), each ordered by an FNV-1a hash of the item id, quotas allocated by largest-remainder so the sample's mix mirrors the pool's; deterministic and reproducible, no clock or randomness, with the append-only table (not the function) guaranteeing a settled pick never leaves; full unit suite over emptiness, whole-pool, determinism, monotonic growth, proportional strata, and remainder seats |
| 2026-07-20 | feat(api): store golden-set audits and the quality query | 2–2.5 | DrizzleGoldenSet (ADR 0051): insert-only sample (addToSample onConflictDoNothing, returns the newly added count), sampleCandidates reducing the direction pool to id/kind/attestation for the sampler, a queueFor that lists sampled items a reviewer has not audited (oldest first) each with the most recent prior audit by anyone, saveAudit upserting one row per (item, reviewer), and quality() computing the mean of the three axes across every audit rescaled 1-5 → 0-100 with a distinct-item count, null when no audits; Testcontainers integration suite over sampling idempotency, the audit upsert, prior-audit context, direction scoping, and the score |
| 2026-07-20 | feat(api): serve golden audit, sampling, and quality routes | 2–2.5 | GET /admin/golden and POST /admin/golden/:itemId/audit (reviewer+): the first hydrates a reviewer's un-audited queue with items and any prior audit, the second upserts three 1-5 axes plus an optional comment and only for sampled items; POST /admin/golden/sample (admin-only) seeds the sample idempotently, targeting min(200, pool) and inserting only ids not already present; GET /stats gains qualityScore (0-100, null before any audit) and qualityAuditedItems, computed by a query alongside the existing counters (ADR 0051); golden store wired at the composition root and stubbed inert in test deps |
| 2026-07-20 | feat(i18n): add golden-set audit and quality strings | 1–1.5 | Twelve sorted keys added to all seven catalogs (govori en+isv, fol en+sq+de+it+tr) for the golden-set benchmark (ADR 0051): the public quality-score line honest about its audited-item count, the reviewer audit view's title and intro, the three axis labels (accuracy/naturalness/fit), the 1-5 scale comment field and save action, the audited-count header, the empty and unavailable states, and the prior-peer-audit context line; good-faith draft translations, parity/sort/non-empty gates green for both instances |
| 2026-07-20 | feat(web): publish the quality score and add the golden-set audit view | 2.5–3 | StatsView shows the golden-set quality score only when non-null and always states how many items back it ("Quality score: 78/100 (34 items audited)"), never dressing a thin sample as a verdict (ADR 0051). New reviewer-only GoldenAuditView mirrors the review-queue card: item text/translation, any prior peer audit as context, three 1-5 axis selectors, an optional comment, and a submit that stays disabled until all three axes are scored and advances to the next item as each audit lands; reached from the account surface beside Review drafts. Client gains the extended stats schema plus fetchGoldenQueue and submitGoldenAudit (comment sent only when non-empty), all failing closed; App and AccountView thread the new view |
| 2026-07-20 | docs: note the golden-set benchmark implementation (adr 0051) | 0.5 | Implementation note appended to ADR 0051: the measured half of quality alongside the preventive gate — the two append-only/upsert tables, the pure deterministic sampler and its idempotent seeding route, the reviewer-gated audit endpoints, and the mean-of-three-axes score served on /stats as a query and shown honestly with its sample size |
